...
void clusterLidarWithROI(std::vector<BoundingBox> *boundingBoxes, const std::vector<LidarPoint> *lidarPoints, const float shrinkFactor, const cv::Mat &P_rect_xx, const cv::Mat &R_rect_xx, const cv::Mat &RT) {
    ...
    for (auto point : *lidarPoints) {
        ...
        for (std::vector<BoundingBox>::iterator box{ begin(*boundingBoxes) }; box != end(*boundingBoxes); ++box) {
            // shrink current bounding box slightly to a having too many outlier points around the edges
            cv::Rect smallerBox;
            smallerBox.x = box->roi.x + shrinkFactor * box->roi.width * 0.5;
            smallerBox.y = box->roi.y + shrinkFactor * box->roi.height * 0.5;

            smallerBox.width = box->roi.width * oneMinusShrinkFactor;
            smallerBox.height = box->roi.height * oneMinusShrinkFactor;
            //boxID = box->boxID;

            // check wether point is within current bounding box
            if (smallerBox.contains(pt)) { enclosingBoxes.push_back(box); }
        }

        // check whether point has been enclosed by one or by multiple boxes
        if (enclosingBoxes.size() == 1) { (*boundingBoxes)[enclosingBoxes.back()->boxID].lidarPoints.push_back(point);}
    }
}
...
bool contains(cv::Rect roi, cv::Point2f point) {

    int left_x = static_cast<int>(roi.x-roi.width/2-20);
    int right_x = static_cast<int>(roi.x+roi.width/2+20);
    int left_y = static_cast<int>(roi.y-roi.height/2-20);
    int right_y = static_cast<int>(roi.y+roi.height/2+20);
    if(point.x > left_x && point.x < right_x){
        if(point.y > left_y && point.y < right_y) {
            return true;
        }
    }
    return false;
...
inline const bool matchPointToBoundingBox(const DataFrame *frame, const int index, int &matchingBoxId) {
    ...
    for (int currentBoundingBoxIndex{ 0 }; currentBoundingBoxIndex < frame->boundingBoxes->size(); currentBoundingBoxIndex++) {
        ...
        //if(!contains(frame->boundingBoxes->at(currentBoundingBoxIndex).roi, point)) { continue; }
